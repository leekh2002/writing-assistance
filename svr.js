...
app.post('/process/getFeedback', async (req, res) => {
  ...
  // GPT API 호출로 피드백 요청
  const result = await callChatGPT(
    `아래 글쓰기 문제에 관해서 작성한 글을 글 전체의 구성, 명확성, 문법적 정확성, 어휘의 풍부성, 문장 구사력, 의도 표현력을 토대로 매우 구체적으로 피드백해줘. 피드백을 받을 부분에 구체적인 예시 또한 제공해줬으면 좋겠어. 또한, 마크다운 형식에 관한 피드백은 절대로 포함하지마. 답변은 반드시 순수json으로만 해주고, 추가 텍스트(\', \`등)는 사용하지마. 예시는 다음과 같아. 총점은 100점 만점으로 하되, 숫자 하나만 적어줘 ex)90/100이 아닌, 90 \n총점: 87\n피드백:{글 전체의 구성: , 명확성: , 문법적 정확성: , 어휘의 풍부성: , 문장 구사력: , 의도 표현력: }}\n이때, 피드백은 강조해야할 부분이나 문단에는 html태그를 붙여줘.\n\n-문제-\n제목: ${problemTitle}\n내용: ${problemContent}\n------\n\n-작성한 글-\n${writing}\n----------`
  );
